    ...
    <Form ref="form" :model="formData" :rules="rules" label-position="top">
      ...
      <div class="item-box">
        ...
        <FormItem label="项目类别" prop="code">
          <div class="service-type" v-if="permiss.create">
            <span>找不到项目类别？</span>
            <span @click="showCateDialog"><i class="iconfont icon-xinzeng"></i>新增项目类别</span>
          </div>
          <Cascader :data="codeData" v-model="formData.code"  :placeholder="cloudPlaceholder"></Cascader>
        </FormItem>
      ...
      <div class="item-box" v-if="formData.pricingType === 'B'" style="width:50%;">
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        ...
        // code: [
        //   { required: true, message: '请选择项目类别', trigger: 'change', type: 'array', validator:validator }
        // ],
        cost: [
  ...
  mounted () {
    this.formData = this.downloadObj
    this.formData.cost = this.formData.cost + ''
    this.cloudPlaceholder = this.formData.cateStr
  },

  destroyed () {
    this.$store.commit('setDownloadObj', {})
  },
  ...
  methods: {
    ...
    transformCode () {
      ...
      if (!code) return
      let arr = [];
      if (Object.prototype.toString.call(this.formData.code) !== '[object Array]') {
        if (code.indexOf('#') != -1) { // 兼容新旧code，聪哥24个字母的锅
          code = code.split('#');
          arr.push('#' + code[1]);
          if (code[2]) {
    ...
    onFooterClick (index) {
      const that = this
      if (index === 0 ) {
        this.submitFn( function () { that.$router.go(0) } )
      }
    },

    submit () {
      const that = this
      this.submitFn( function () { that.$router.push('/store/service/list') } )
    },
