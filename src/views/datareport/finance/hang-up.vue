<template>
  ...
  <div class="datareport-list">
    ...
    <w-table
      class="table-width credit-table"
      ref="table"
...
import { CREDIT_EXPORT } from '@/https/api';
import { getLabel, customerType,onCreditStatus } from '@/store/json/default';
...
export default {
  data () {
    ...
    let columns = [
      ...
      { title: '挂账金额', key: 'receivable' },
      { 
        title: '挂账状态', 
        key: 'onCreditStatus',
        render: (h, { row }) => {
          return h('div', row.onCreditStatus === '0' ? '未收' : '已收');
        }
      },
      { title: '操作人', key: 'operatedBy' },
      { title: '挂账开始时间', key: 'creditDate' },
      { 
        title: '挂账结束时间', 
        key: 'settlementDate',
        render: (h, { row }) => {
    ...
    return {
      ...
      dataList: [
        { title: '挂账总金额', value: '0', unit: '元' },
        { title: '已收挂账金额', value: '0', unit: '元' },
        { title: '未收挂账金额', value: '0', unit: '元' },
      ],
      btns: [
        { name: '今日', value: 'today' },
  ...
  watch: {
    tableData (value) {
      if (value) {
        this.dataList[0].value = value.receivable;
        this.dataList[1].value = value.realCost;
        this.dataList[2].value = value.unreceived;
      }
    },
  ...
  methods: {
    ...
    onQuery (params) {
      ...
      this.params.onCreditStatus = params.onCreditStatus;
    ...
    exportData (type) {
      ...
      const url = CREDIT_EXPORT + '?token=' + this.$storage.get('TOKEN') + '&t=' + type;
      let string = '';
      for (let i in params) {
        if (params[i]) {
