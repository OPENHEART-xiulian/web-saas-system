    ...
    methods: {
      ...
      setDefault (list) {
        ...
        if (this.data.warehouseId && !hasWarehouse && this.data.isEdit) {
          allOriginQuantity = Number(allOriginQuantity) + Number(this.data.quantity);
          let wrehouse = {
          ...
          warehouseId = hasDefault ? hasDefault.warehouseId : (list[0] ? list[0].warehouseId : '');
        }
      ...
      onChange (v, isDefault) {
        ...
        if (data) {
          ...
          if (Number(this.data.quantity) > Number(this.data.originQuantity) && this.data.stockOutType === '1') {
