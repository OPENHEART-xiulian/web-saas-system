  ...
  <div class="workorder-detail">
    <saas-bread title="工单详情">
      <template slot="actions">
        ...
        <Button v-if="print" type="error" @click="openPrint">打印</Button>
      </template>
    </saas-bread>
    <div class="top">
    ...
    >
      ...
      <p class="msg">
        ...
        <span v-if="workDetail.serviceAdvisor && workDetail.serviceAdvisor.userId && billUsers.length>0">服务顾问：{{ getUserName(workDetail.serviceAdvisor.userId) }}</span>
...
export default {
    ...
    return {
      ...
      projectColumns: [
        ...
        { title: '金额 (元)', key: 'cost' },
        {
          ...
          key: 'cost',
          render: (h, { row }) => {
            return h('div', row.techniciansName||"")
          }
        },
        {
          title: '备注信息',
  ...
  created () {
    ...
    let { workerList, workerCleanout, workerSparepart } = this.$permission.data
    let str = 'toGetWorkDetail'
    let params = {}
    if (this.$route.query.bussiness == 1) {
      str = 'toGetErecordDetail'
      params = { id: this.id }
      this.print = true
    } else {
      let request = ''
      switch (this.type) {
        case 'worker':
          request = 'workDetail'
          this.print = workerList.print
          break
        case 'sparepart':
          request = 'saleDetail'
          this.print = workerSparepart.print
          break
        case 'cleanout':
          request = 'careDetail'
          this.print = workerCleanout.print
          break
        default:
          break
      }
      params = {
        params: {
          statementNo: this.id || '',
          shareStoreId: this.$route.query.shareStoreId&&this.$route.query.shareStoreId!='undefined'?this.$route.query.shareStoreId : ''
        },
        request: request
      }
    }
  ...
  methods: {
    ...
    getUsers (arr = []) {
      ...
      arr.map(itemc => {
        let user = this.billUsers.find(itemBill => itemBill._id === itemc);
        if (user) users.push(user.username)
      })
      return users.join(',')
    },
    getUserName (userid) {
      let user = this.billUsers.find(item => item._id == userid);
