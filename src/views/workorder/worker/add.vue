<template>
    <div class="add-form add-page workAddPage">
            ...
            <div class="search-form vehicle">
                <div class="form-box width-20">
                    ...
                    <i-form-item class="form-item" label="故障信息" style="width: 40%">
                        <i-input v-model="formData.description" :rows="3" type="textarea"
                                 placeholder="输入故障信息描述"></i-input>
                    </i-form-item>
                </div>
...
<script>
    ...
    export default {
        ...
        data() {
            ...
            const validateDelivery = (rules, value, callback) => {
                if (Number(that.formData.adviceMileage) === -1) {
                    if (rules.required && value === "") {
                        return callback(rules.message);
                    }
                    // 格式错误的时候 提示错误
                    ...
                    if (Number(that.formData.adviceMileage) - Number(that.formData.deliveryMileage) < 101 && Number(that.formData.adviceMileage) - Number(that.formData.deliveryMileage) > -1) {
                        rules.message = `车辆建议保养里程：${that.formData.adviceMileage}，请提醒客户保养！`
                        return callback(rules.message);
                    }
                }
                callback();
        ...
        methods: {
            ...
            customerSelectChange(data) {
                ...
                if (data.isUpdateCustomer) {
                    ...
                    this.formData.adviceMileage = String(data.vehicleList[0].adviceMileage) || -1
                }
            ...
            submit() {
                ...
                if (!this.state) {
                    ...
                    // 验证VINcode
                    let VINCode = this.$refs.customerSelect.getSelectedVIN()
                    if (VINCode) {
                        if (!this.verify('VINCODE', VINCode)) {
                            this.$Message.warning("车辆VIN码错误")
                            return
                        }
                    }
...
<style lang="less">
    .workAddPage .ivu-date-picker-cells {
        width: auto;
    }
</style>
