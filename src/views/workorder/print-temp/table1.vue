  ...
  <div>
    <div class="print-form">
      ...
      <table style="border-top: 1px solid #000;">
        ...
        <tr>
          <td colspan="2" class="print-form__label">送修日期</td>
        ...
        <tr>
          ...
          <td colspan="4">{{formData.factoryMileage || '--'}}km</td>
        </tr>
        <tr>
          ...
          >
            <table>
              ...
              <tr class="no-border" v-if="needRemark">
                <td colspan="12">
                  <div v-if="!isRemark">6、{{formData.workOrderRemark}}</div>
              ...
              <tr class="no-border" v-if="formData.QRCode">
                <td colspan="12">{{isRemark?"7":"6"}}、可扫描二维码，查看本结算清单是否已按规定上传。<br>
                <img :src="formData.QRCode" style="height: 100px;width: 100px;display: block;margin: 10px">
                </td>
              </tr>
              <tr class="no-border">
...
export default {
  ...
  methods: {
    computedCost (formData) {
      ...
      const payLog = this.formData.payInfo
      const realPayInfo = payLog.pop()

      let realCost = 0
      let advanceCost = 0
      payLog.forEach(item => {
          advanceCost += Number(item.amount || 0)
      })
      if(realPayInfo&&realPayInfo.amount) {
        realCost += realPayInfo.amount;
      }
