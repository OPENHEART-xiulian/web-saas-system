<template>
	<!-- 打印模板 -->
  <div class="print-model-bg" v-if="show">
    <div class="print-model clear">
      <div class="print-bg clear">
        <div class="print-area" ref="printForm">
          <h2 class="title">采购单</h2>
          <p class="statement-no">工单号：{{detailData.purchaseCode || '--'}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;开单时间：{{detailData.orderAt || '--'}}</p>
          <div class="print-form">
            <table>
              <tr>
                <td style="width: 76px;">录单人</td>
                <td>{{detailData.operatedBy || '--'}}</td>
                <td>总金额</td>
                <td>{{detailData.totalCost || '--'}}</td>
                <td>数量</td>
                <td>{{detailData.totalQuantity || '--'}}</td>
              </tr>
              <tr>
                <td>供应商</td>
                <td colspan="5">{{detailData.supplierName || '--'}}</td>
              </tr>
              <!-- 为了兼容样式最后一行的border -->
              <tr></tr>
            </table>

            <table>
              <tr>
                <td style="width: 76px;" :rowspan="detailData.components ? (detailData.components.length > 1 ? detailData.components.length+1 : 2) : 2">配件信息</td>
                <td style="width: 53px;">序号</td>
                <td style="width: 90px;">配件编号</td>
                <td style="width: 90px;">原厂编码</td>
                <td style="width: 90px;">名称</td>
                <td style="width: 80px">品牌</td>
                <td style="width: 70px">数量</td>
                <td style="width: 90px">单价(元)</td>
                <td style="width: 90px">金额(元)</td>
              </tr>
              <tr v-if="!detailData.components || detailData.components.length==0">
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
              </tr>
              <tr v-for="(item,index) in detailData.components" :key="index">
                <td class="index">{{index+1}}</td>
                <td>{{item.partNo || '--'}}</td>
                <td>{{item.partFactoryNo || '--'}}</td>
                <td>{{item.partName || '--'}}</td>
                <td>{{item.brand || '--'}}</td>
                <td>{{item.quantity || '0'}}</td>
                <td>{{item.purchasePrice | getFixed}}</td>
                <td>{{item.totalCost | getFixed}}</td>
              </tr>
              <tr>
                <td>合计：</td>
                <td colspan="8">{{detailData.totalCost | getFixed}}元</td>
              </tr>
              <!-- 为了兼容样式最后一行的border -->
              <tr></tr> 
            </table>

            <table>
              <tr>
                <td style="width: 76px;">备注</td>
                <td colspan="3">{{ detailData.remark || '--' }}</td>
              </tr>
              <tr>
                <td style="width: 76px;">采购人</td>
                <td>{{ detailData.operatedBy || '--' }}</td>
                <td>打印日期</td>
                <td>{{ time }}</td>
              </tr>
            </table>
          </div>
        </div>
      </div>
    </div>
    <div class="bottom-btn">
      <div>
        <i-button type="default" @click.stop="closePrint">取消</i-button>
        <i-button type="primary" @click.stop="print">打印</i-button>
      </div>
    </div>
  </div>
</template>

<script>
import { mapGetters } from 'vuex';
import Print from '@/assets/js/print.js';
import { formatTime } from '@/assets/js/utils';
import '@/../public/css/print.css';

export default {
  data () {
    return {
      show: false,
      time: formatTime(new Date(), 'yyyy-MM-dd')
    }
  },

  computed: {
    ...mapGetters({
      detailData: 'getPurchaseDetail'
    })
  },

  methods: {
    openPrint (id) {
      this.$store.dispatch({
        type: 'toGetPurchaseDetail',
        params: {
          purchaseId: id
        }
      })

      this.show = true;
    },
    closePrint () {
      this.show = false;
    },
    print () {
      const newPrint = new Print(this.$refs.printForm, '/css/print.css');
    },
  },

  filters: {
    getFixed (v) {
      if (!v) return '0.00';
      return Number(v).toFixed(2);
    },
  }
}
</script>
