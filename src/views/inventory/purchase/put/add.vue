    ...
    <div class="box-shadow">
      <i-form ref="form" :model="formData" :rules="rules" class="refund-form" label-position="top">
        <div class="form-box">
          ...
          <FormItem label="采购人" prop="operatorId">
      ...
      <part-component ref="part" class="part-list" :auto-init="false" @on-change="partChange" />
  ...
  export default {
    ...
    methods: {
      ...
      async submit () {
        ...
        if (!this.cerifyPurchasePrice(formData.components)) {
          this.$Message.warning('采购单价不能为0');
          return
        }
        if (isValid) {
          this.loading = true;
          let requert = "purchaseAdd";

          if (!this.status) {
            requert = "purchaseUpdate";
            formData.purchaseId = this.$route.params.id;
          };
          const res = await this.$store.dispatch({
