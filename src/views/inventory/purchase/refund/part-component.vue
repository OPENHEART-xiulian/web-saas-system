    ...
    <table class="input-list-table">
      <thead class="theader" v-if="formRows.length > 0">
        <tr class="row">
          ...
          <th class="col" style="width: 100px">库存</th>
          <th class="col" style="width: 100px">退货数量</th>
          <th class="col" style="width: 100px">退货单价</th>
          <th class="col" style="width: 100px">退货金额</th>
      ...
      <tbody class="tbody">
        <template v-if="!loading">
          <tr class="row" v-for="(item, index) in formRows" :key="item.componentId">
            ...
            <!-- 库存 -->
            <td class="col">
              <Tooltip placement="top" :content="formRows[index].warehouseQuantity">
                <span>{{formRows[index].warehouseQuantity}}</span>
              </Tooltip></td>
            <!-- 退货数量 -->
            <td class="col">
              <format-input 
                v-model="formRows[index].quantity" 
                :max="formRows[index].warehouseQuantity"
                :min="0.01"
            ...
            <!-- 退货单价 -->
            <td class="col">
              <format-input v-model="formRows[index].price" :min="0" @on-blur="getRowCost(index)" reg="PRECISION"/>
            </td>
            <!-- 退货金额 -->
            <td class="col">
              <Tooltip placement="top" :content="formRows[index].cost | toFixed">
                <span>{{item.cost | toFixed}}</span>
              </Tooltip>
            </td>
      ...
      <tfoot class="tfooter">
        <tr>
          <td colspan="11">
            <i-button type="primary" @click="openChoose">选择配件</i-button>
          </td>
        </tr>
      </tfoot>
    </table>
    <choose-dialog ref="choose" request="taskstockParts" :params="{includeEmpty: 0}" @choose-success="distinct"></choose-dialog>
  </div>
</template>
...
<script>
  ...
  import { getLabel } from '@/store/json/default.js';
  export default {
    ...
    methods: {
      ...
      initList (to) {
        ...
        to.forEach(item => {
          ...
          item.price = item.price || item.purchasePrice || '0';
          const cost = multiply(item.quantity, item.price );
      ...
      getRowCost (index, type) {
        ...
        const cost = multiply(quantity, price);
      ...
      distinct (array) {
        array.forEach(item => {
          ...
          item.quantity = '1';
          this.formRows.forEach((citem, index) => {
            if (citem.partNo === item.partNo) {
              item.quantity = citem.quantity;
              item.retailPrice = citem.retailPrice;
              item.warehouseQuantity = citem.warehouseQuantity;
              // this.formRows[index] = citem;
            }
          })
        })
        this.initList(array);
      ...
      ckChange (data, index) {
        ...
        this.formRows[index].warehouseQuantity = data.quantity || '0';
