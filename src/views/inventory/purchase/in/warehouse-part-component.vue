<template>
  ...
  <div style="position: relative" class="part-component">
    <table class="input-list-table">
      <thead class="theader" v-if="formRows.length > 0">
        <tr class="row">
          ...
          <th class="col">品牌</th>
          <th class="col">单位</th>
          <th class="col" style="width: 100px"  v-if="!isCommon">商品核对</th>
      ...
      <tbody class="tbody">
        <template v-if="!loading">
          <tr class="row" v-for="(item, index) in formRows" :key="item.componentId">
            ...
            <!-- 商品核对 -->
            <td v-if="!isCommon" @click="selectRealName(index)" style="cursor:pointer;"> <i-button style="margin-right:10px;">{{item.realWareHouseName || '请选择'}}</i-button> </td>
            <!-- 仓库 -->
            <td class="col">
            ...
            <td class="col">
              <format-input 
                v-model="formRows[index].incQuantity" 
                :min="1"
            ...
            <!-- 配件单价 -->
            <td class="col">
              <format-input v-model="formRows[index].purchasePrice" :min="0.01" :disabled="!isCommon" @on-blur="getRowCost(index)" reg="PRECISION"/>
            </td>
            <!-- 采购金额 -->
    ...
    <choose-single-dialog ref="chooseSingle" @choose-success="distinctSingle" />
  </div>
</template>
...
<script>
  ...
  import ChooseSingleDialog from '@/components/dialog/choose-maintain-singleSelect-dialog ';
  ...
  export default {
    components: { ChooseDialog, CKSelect, ChooseSingleDialog },
    props: {
      autoInit: {
        type: Boolean,
        default: true
    ...
    methods: {
      ...
      initList (to) {
        ...
        to.forEach(item => {
          ...
          if(this.isCommon){
            ...
            item.incQuantity = item.quantity||'0';
      ...
      selectRealName (index) {
        this.partItemIndex = index
        let formRows = JSON.parse(JSON.stringify(this.formRows));
        const arr = [] 
        if(formRows[index].realPart.componentId){
          arr.push(formRows[index].realPart)
        }
        this.$refs.chooseSingle.openModel(arr)
      },
      distinct (array) {
      ...
      distinctSingle(array){
        this.formRows[this.partItemIndex].componentId = array[0].componentId
        this.formRows[this.partItemIndex].warehouseId = array[0].warehouseId
        this.formRows[this.partItemIndex].realWareHouseName = array[0].partName
        this.formRows[this.partItemIndex].realPart = array[0]
        this.$forceUpdate();
      },
      getFormRows () {
