  ...
  <div class="refund-add">
    ...
    <div class="box-shadow">
      <i-form ref="form" :model="formData" :rules="rules" class="refund-form" label-position="top">
        ...
        <div class="form-box" v-if="formData.stockInType === '3'">
          <FormItem label="调拨申请单号" prop="allotApplyNo">
      ...
      <Warehouse-part-component ref="part" class="part-list" :auto-init="false" @on-change="partChange" :isCommon="isCommon"/>
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        rules: {
          allotApplyNo: [{ required: true, message: '请选择出库单号' }],
        },
        storageType
      }
    },
    ...
    computed: {
      isCommon(){
        return this.formData.stockInType === '0'? true : false
      }
    },

    watch: {
      'formData.stockInType':function(n,o){
        if(n === '0'){
          this.initFormDataFn()
        }else{
          this.formData.items = []
      ...
      async searchNumber(e){
        try {
          ...
          if (res.code === '0') {
            ...
            this.formData.stockOutNo = res.data.stockOutNo
            this.formData.callOutStoreId = res.data.callOutStoreId
            this.$refs.part.setDefaultList(this.formData.items);
          }else{
